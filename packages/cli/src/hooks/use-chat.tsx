import type { Mode } from "@zeocode/database";
import {
...
export function useChat(sessionId: string, initialMessages: Message[]) {
	...
	const handleStream = useCallback(
		async (response: ClientResponse<unknown>, activeStream: ActiveStream) => {
			...
			if (!response.body) {
				updateMessages((prev) => [
					...prev,
					{
						id: crypto.randomUUID(),
						role: "error",
						content: "Empty streaming response body",
					},
				]);
				return;
			}
			...
			for await (const { data } of stream) {
				...
				let event: z.infer<typeof chatStreamEventSchema>;
	...
	// Auto resume if the conversation ends with a user message with no reply
